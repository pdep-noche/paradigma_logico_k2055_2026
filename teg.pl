...
elQueTieneMasEjercitos(Jugador, Pais):-ocupa(Pais, Jugador, CantEjer), 
    forall(ocupa(_, _, Ejercitos), Ejercitos =< CantEjer).
...
cumpleObjetivos(Jugador):-jugador(Jugador), forall(objetivo(Jugador, Objetivo), cumple(Objetivo, Jugador)).

cumple(ocuparContinente(Continente), Jugador):- ocupaContinente(Jugador, Continente).
cumple(ocuparPaises(Cant, Continente), Jugador):- cantPaises(Jugador, Continente, Cantidad), Cant =< Cantidad.
cumple(destruirJugador(JugadorADestruir), _):- not(ocupa(_, JugadorADestruir, _)).

cantPaises(Jugador, Continente, Cant):- jugador(Jugador), 
    findall(Pais, estaOcupado(Pais, Jugador,Continente ), ListaPaises), 
    length(ListaPaises, Cant).

estaOcupado(Pais, Jugador, Continente):- ocupa(Pais, Jugador, _), estaEn(Pais, Continente).
